...
// ======================================================================
//
// doctest.h - the lightest feature-rich C++ single-header testing framework for unit test and TDD
//
...
struct ContextOptions //!OCLINT too many fields
{
    ...
    String   order_by;  // how test should be ordered
    unsigned rand_seed; // the seed for rand ordering

    unsigned first; // the first (matching) test to be executed
    unsigned last;  // the last (matching) test to be executed

    int abort_after;           // stop test after this many failed assertions
    int subcase_filter_levels; // apply the subcase filters for the first N levels

    bool success;              // include successful assertions in output
    bool case_sensitive;       // if filtering should be case sensitive
    bool exit;                 // if the program should be exited after the test are ran/whatever
    bool duration;             // print the time duration of each test case
    bool minimal;              // minimal console output (only test failures)
    bool quiet;                // no console output
    bool no_throw;             // to skip exceptions-related assertion macros
    bool no_exitcode;          // if the framework should return 0 as the exitcode
    bool no_run;               // to not run the test at all (can be done with an "*" exclude)
    ...
    bool count;            // if only the count of matching test is to be retrieved
    bool list_test_cases;  // to list all test matching the filters
...
// for registering test
#define DOCTEST_TEST_CASE(decorators)                                                              \
    DOCTEST_CREATE_AND_REGISTER_FUNCTION(DOCTEST_ANONYMOUS(DOCTEST_ANON_FUNC_), decorators)

// for registering test in classes - requires C++17 for inline variables!
...
// for registering test with a fixture
...
// for grouping test in test suites by using code blocks
...
// for registering test in classes
...
using ticks_t = timer_large_integer::type;
    ...
    // this holds both parameters from the command line and runtime data for test
...
namespace {
    ...
    // for sorting test by suite/file/line
    ...
    // for sorting test by name/suite/file/line
...
namespace detail {
    // used by the macros for registering test
    ...
    struct JUnitReporter : public IReporter
    {
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            xml.startElement("testsuite").writeAttribute("name", binary_name)
                    ...
                    .writeAttribute("test", p.numAsserts);
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        void printHelp() {
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "c,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "count                         "
              << Whitespace(sizePrefixDisplay*1) << "prints the number of matching test\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ltc, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-test-cases               "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching test by name\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tc,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-case=<filters>           "
              << Whitespace(sizePrefixDisplay*1) << "filters     test by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tce, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-case-exclude=<filters>   "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT test by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sf,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file=<filters>         "
              << Whitespace(sizePrefixDisplay*1) << "filters     test by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sfe, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file-exclude=<filters> "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT test by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ts,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-suite=<filters>          "
              << Whitespace(sizePrefixDisplay*1) << "filters     test by their test suite\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tse, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "test-suite-exclude=<filters>  "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT test by their test suite\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ob,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "order-by=<string>             "
              << Whitespace(sizePrefixDisplay*1) << "how the test should be ordered\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "e,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "exit=<bool>                   "
              << Whitespace(sizePrefixDisplay*1) << "exits after the test finish\n";
...
int Context::run() {
    ...
    if(!testArray.empty()) {
        if(p->order_by.compare("file", true) == 0) {
            ...
            // means no sorting - beneficial for death test which call into the executable
    ...
    for(auto& curr : testArray) {
        ...
        // do not execute the test if we are to only count the number of filter passing test
        if(p->count)
            continue;
...
DOCTEST_MSVC_SUPPRESS_WARNING_POP
            ...
            // stop executing test if enough assertions have failed
