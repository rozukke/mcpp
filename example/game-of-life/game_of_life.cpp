...
Life::Life(int width, int depth)
    : width(width), depth(depth), isRunning(false), delay(250) {
    buildPosition = mc.getPlayerPosition();
    buildPosition.y -= 5;
    buildPosition.x -= width / 2;
    buildPosition.z -= depth / 2;

    bounds = mcpp::Coordinate(buildPosition.x + width - 1, buildPosition.y,
                              buildPosition.z + depth - 1);
    upperBounds =
        mcpp::Coordinate(buildPosition.x + width - 1, buildPosition.y + 1,
                         buildPosition.z + depth - 1);
    upperBuildPosition =
        mcpp::Coordinate(buildPosition.x, buildPosition.y + 1, buildPosition.z);
...
void Life::Update() {
    ...
    for (int x = 0; x < width; x++) {
        for (int z = 0; z < depth; z++) {
            ...
            mcpp::Coordinate position(upperBuildPosition.x + x,
                                      upperBuildPosition.y,
