...
Model::Model(const std::string &filename)
{
    ...
    while(std::getline(_file, line))
    {
        ...
        else if(line.substr(0, 2) == "f ")
        {
            ...
            {
                std::sscanf(line.c_str(), "f %d//%*d %d//%*d %d//%*d", &face.first, &face.second, &face.third);
            }
            else if(line.find(std::string("/")) != std::string::npos)
            {
                std::sscanf(line.c_str(), "f %d/%*d/%*d %d/%*d/%*d %d/%*d/%*d", &face.first, &face.second, &face.third);
            }
            else
            {
...
void Model::BuildPointCloud(mcpp::MinecraftConnection &mc)
{
    for(const Vec3 &vertex : _vertices)
    {
        mcpp::Coordinate blockPosition = mcpp::Coordinate(_position.x + vertex.x, _position.y + vertex.y, _position.z + vertex.z);
    ...
    for(int x = min.x; x < max.x; x++)
    {
        for(int y = min.y; y < max.y; y++)
        {
            for(int z = min.z; z < max.z; z++)
            {
                ...
                {
                    mcpp::Coordinate blockPosition = mcpp::Coordinate(_position.x + x, _position.y + y, _position.z + z);
...
bool Model::IsWithin(Vec3 &position)
{
    ...
    for(Vec3 direction : directions)
    {
        ...
        for(const Face &face : _faces)
        {
            ...
            if(std::abs(det) < EpsilonFloat) continue; // parallel

            float f = 1.0 / det;
            Vec3 s = sub(position, v1);
            float u = f * dot(s, h);

            if(u < 0.0 || u > 1.0) continue; // outside face

            Vec3 q = cross(s, edge1);
            float v = f * dot(direction, q);

            if(v < 0.0 || u + v > 1.0) continue; // outside face

            float t = f * dot(edge2, q);

            if(t > EpsilonFloat) intersects++;
        }

        if(intersects % 2 == 1) crosses++;
    }
