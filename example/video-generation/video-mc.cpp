...
class Video {
  public:
    Video(const std::string& filename, int width, int height, int scaleFactor,
          int frameRate);
    ~Video() {
        if (_file.is_open()) {
            _file.close();
...
int main(int argc, char* argv[]) {
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0]
                  << " filename.rgb width height scale frame_rate" << std::endl;
        return 1;
    }

    std::string filename = argv[1];
    if (filename.find(".mp4") != std::string::npos) {
        std::cerr << "Be sure to convert mp4 files to rgb files before use."
                  << std::endl
                  << "Usage: ffmpeg -i filename.mp4 -f rawvideo -pix_fmt rgb24 "
                     "filename.rgb"
                  << std::endl;
        return 1;
    }
...
Video::Video(const std::string& filename, int width, int height,
             int scaleFactor, int frameRate)
    : _width(width), _height(height), _scaleFactor(scaleFactor),
      _frameRate(frameRate), _frameDelay(1000 / _frameRate),
      _frameSize(_width * _height * 3) {
...
void Video::Play(mcpp::MinecraftConnection& mc) {
    ...
    _position.z -=
        std::max((_width / _scaleFactor) / 2, (_height / _scaleFactor) / 2);
        ...
        if (!frame.empty()) {
            for (size_t i = 0; i < frame.size(); i += _scaleFactor) {
                ...
                if (index > 0) {
                    ...
                    if (pixel.r == lastPixel.r && pixel.g == lastPixel.g &&
                        pixel.b == lastPixel.b) {
                        continue;
                    }
                }
