...
int main() {
    ...
    int board[size_x][size_z]; // Defaults board states to 0 (meaning no mines nearby)
    ...
    std::vector<mcpp::BlockType> minecols = 
        {mcpp::Blocks::WHITE_CONCRETE, mcpp::Blocks::LIGHT_BLUE_CONCRETE, mcpp::Blocks::CYAN_CONCRETE,
         mcpp::Blocks::GREEN_CONCRETE, mcpp::Blocks::LIME_CONCRETE, mcpp::Blocks::YELLOW_CONCRETE,
         mcpp::Blocks::ORANGE_CONCRETE, mcpp::Blocks::RED_CONCRETE, mcpp::Blocks::BROWN_CONCRETE, mcpp::Blocks::TNT};
        ...
        if (board[x][z] == 9) i--;
            ...
            for (int xmod = -1; xmod < 2; xmod++){
                for (int zmod = -1; zmod < 2; zmod++){
                    if ((x + xmod) >= 0 && (x + xmod) < size_x && (z + zmod) >= 0 && (z + zmod) < size_z ) {
                            ...
                            for (size_t pos = 0; pos < zc.size(); pos++) {
                                ...
                                for (int xmod = -1; xmod <= 1; xmod++) {
                                    for (int zmod = -1; zmod <= 1; zmod++){
                                        if (cord.x + xmod >= orig.x && cord.x + xmod <= corner.x && cord.z + zmod >= orig.z && cord.z + zmod <= corner.z ){
                                            tpos = cord;
                                            tpos.x += xmod;
                                            tpos.z += zmod;

                                            if (state[tpos.x-orig.x][tpos.z-orig.z] != 1) {
                                                clearsToWin--;
                                                if (board[tpos.x-orig.x][tpos.z-orig.z] == 0) {
                                                    zc.push_back(tpos);
                                                }
                                            }
                                            mc.setBlock(tpos,minecols.at(board[tpos.x-orig.x][tpos.z-orig.z]));
                                            state[tpos.x-orig.x][tpos.z-orig.z] = 1;
                                        }
                                    }
                                }
                            }
